...
def main() -> None:
    ...
    with guide_tab:
        guide_count_before = len(st.session_state.get("saved_guides", []))
        render_study_tab(api_key, subject, workspace, study_mode)
        save_active_workspace_to_db(current_user, subject, workspace)
        # Fire event when a new guide was saved this render cycle
        if len(st.session_state.get("saved_guides", [])) > guide_count_before:
            capture("study_guide_generated", current_user, {
                "subject": subject,
                "study_mode": study_mode,
                "total_guides_saved": len(st.session_state.get("saved_guides", [])),
            })

    with quiz_tab:
        quiz_history_before = len(workspace.get("quiz_history", []))
        render_quiz_tab(api_key, subject, workspace)
        save_active_workspace_to_db(current_user, subject, workspace)
        # Fire event when a quiz attempt was just submitted this render cycle
        quiz_history_after = len(workspace.get("quiz_history", []))
        if quiz_history_after > quiz_history_before:
            latest = workspace["quiz_history"][-1]
            capture("quiz_attempt_completed", current_user, {
                "subject": subject,
                "score": latest.get("score", 0),
                "total_questions": len(latest.get("questions", [])),
                "study_mode": study_mode,
            })
