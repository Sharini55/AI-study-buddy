...
def _missed_questions_for_topic(quiz_history: list[dict], topic: str) -> list[dict]:
    """
    Pulls every historically-missed question on `topic` out of quiz_history,
    so the Act step can hand generate_remediation_pooled() real, specific
    context instead of just a bare topic name.

    Falls back to a generic placeholder when the agent picked this topic on
    low-confidence grounds (few attempts) rather than because it was missed —
    there may be no recorded misses to pull from yet.
    """
    found = [
...
def _save_agent_guide(subject: str, topic: str, content: str) -> None:
    """Same save pattern tabs/study.py uses for user-generated guides, so the
    agent's output survives navigation instead of vanishing when quiz_key
    state resets."""
...
def render_agent_panel(api_key: str, subject: str, workspace: dict) -> None:
    """
    The autonomous loop's UI surface: PERCEIVE + SCORE + DECIDE happen every
    render via loop_status(); ACT happens only when the user clicks the
    button (Gemini calls aren't free, so we don't auto-fire them); EVALUATE
    happens through the normal quiz-taking flow below, reusing the exact
    same grading code a manual quiz uses; ADAPT + LOOP happen for free on
    the next render, since compute_mastery() re-reads quiz_history fresh
    every time and will reflect whatever the student just did.
    """
            ...
            with st.spinner(f"Generating targeted guide for {target}…"):
                try:
                    guide = generate_remediation_pooled(
                        api_key, subject, workspace, missed_for_topic,
...
def render_quiz_tab(api_key: str, subject: str, workspace: dict) -> None:
    ...
    render_agent_panel(api_key, subject, workspace)
    ...
    if submitted and st.session_state[post_quiz_key]:
        ...
        if st.session_state.get(f"agent_quiz_{wid}"):
            # This quiz was the agent's own Act step, not a manually-requested
            # one — the manual "what do you want to do next" menu below is
            # for the human-driven flow and doesn't belong here. The agent
            # panel at the top of this page already recomputed mastery from
            # this result (ADAPT) the moment quiz_history updated; all this
            # screen needs to do is hand control back to it (LOOP).
